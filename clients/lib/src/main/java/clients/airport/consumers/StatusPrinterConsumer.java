...
public class StatusPrinterConsumer extends AbstractInteractiveShutdownConsumer {
  ...
  public void run() {
    ...
    try (KafkaConsumer<Integer, TerminalInfo> consumer =
        new KafkaConsumer<>(props, new IntegerDeserializer(), new TerminalInfoDeserializer())) {
  ...
  /**
   * Runs the consumer until the user presses Enter. This works around a limitation in Eclipse that
   * Ctrl+C cannot be sent from the Console view. Gradle's JavaExec task has a similar limitation.
   */

...
/**
 * Simulates an airport terminal, with a number of self-checkin desks. These desks have customers
 * start checkin processes randomly. They usually complete them, but they may sometimes cancel the
 * process.
 *
 * <p>The checkin desks are not very reliable: sometimes they simply stop responding, or they may
 * have a hardware issue or may run out of paper. A checkin desk uses a piece of paper per customer,
 * and will give out a "low paper" warning before it completely runs out of paper.
 *
 * <p>This simulator accepts a 'speed up factor', which can be used to uniformly scale all the waits
 * and intervals between events. By default, it's 1 (no scaling).
 */
public class AirportSimulator {
  ...
  public class Terminal {
    ...
    private void computeCheckinEnd() {
      final int checkinSeconds =
          CHECKIN_MINIMUM_SECONDS
              + random.nextInt(CHECKIN_MAXIMUM_SECONDS - CHECKIN_MINIMUM_SECONDS);
  ...
  public void runFor(Duration d) throws InterruptedException {
    final Instant endDate = Instant.now().plus(d);

    while (Instant.now().isBefore(endDate)) {
      for (Terminal terminal : terminals) {
        terminal.tick();
      }
      Thread.sleep(1000 / speedupFactor);
    }
  }
  ...
  protected void fireEvent(EventType type, Terminal t) {
    System.out.printf(
        "Terminal %s at time %s: generated event %s%s%n",
        t.getId(), Instant.now(), type, t.isDuringCheckin() ? " (during check-in)" : "");
  }

  /** Simple program to try out the simulator. */
  public static void main(String[] args) throws InterruptedException {
    AirportSimulator sim = new AirportSimulator(20, new Random(1));

    // To run at 10X speed, uncomment this line
    sim.setSpeedupFactor(10);

    // Runs as many ticks as possible for 5 wall-clock minutes
    sim.runFor(Duration.ofMinutes(5));
  }
}
